...
namespace Tracking_System___Api.Repositories.TrackingRepo
{
    public class TrackingRepo : ITrackingRepo
    {
        ...
        {
            var location1 = new location() { TripId = location.TripId, longitude = location.longitude, latitude = location.latitude, accuracy = location.accuracy, Trip = context.Trips.FirstOrDefault(x => x.Id == location.TripId) };
